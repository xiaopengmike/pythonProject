...
def itemApiResIntoDb(newsResult,tdxMarketCode):
    ...
    # id = uuid_[:13] + str(1000000 + res_tittle)[1:]
    ...
    if(res_code):
        # if(res_Type):
            sql = "INSERT INTO app_config_shares_news_info (id,content,title,code,time,stockName,market,created_by,last_modified_by,gmt_create,gmt_modified) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s') ON duplicate KEY UPDATE title = title " % (id,res_content,res_tittle,res_code,res_time,res_stockName,res_market,created_by,last_modified_by,gmt_create,gmt_modified)
            cursor02.execute(sql)
            connection02.commit()

for newsResult in tdxNewsDictLi:
    try:itemApiResIntoDb(newsResult,tdxMarketCode)
    except:continue
