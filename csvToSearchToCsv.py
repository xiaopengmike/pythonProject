...
#从csv读
# csvFile = open("lc_news.csv", "r",encoding='utf-8')
csvFile = open("lc_news.csv", "r")
dict_reader = csv.DictReader(csvFile)

newsLi = []

for row in dict_reader:
    row['contentStockCountDict']='1'
    # print(row)
    newsLi.append(row)
# print(len(newsLi))
...
stockSearchWordLi = ['ChiNameAbbr', '平安银行', '畅联股份', '万科', '金田实业', '国农科技', '世纪星源', '深振业', '全新好', '神州高铁', '中国宝安',
                     '美丽生态',
                     '深物业',
...
for newsItem in newsLi:
    # newsItem=strStockCount(newsItem, 0)
    strStockCount(newsItem, 0)


# 写入OK了
csvFile = open("lc_news.csv", "w")
fileheader = ["ID", "InfoTitle", "content", "contentStockCountDict"]
dict_writer = csv.DictWriter(csvFile, fileheader)
dict_writer.writerow(dict(zip(fileheader, fileheader)))

for newsItem in newsLi:
    dict_writer.writerow(newsItem)